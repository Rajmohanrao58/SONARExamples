...
public class ByteCodeResourceLocator implements BatchExtension {
    ...
    public InputFile buildInputFile(String fileName,FileSystem fs) {
        for(String sourceDir : SOURCE_DIRECTORIES) {
        	...
        	LOG.info("Source file tested : "+sourceDir+"/"+fileName);
            Iterable<InputFile> files = fs.inputFiles(fs.predicates().matchesPathPattern(sourceDir+"/"+fileName));
            for (InputFile f : files) {
            	LOG.info("Input Files : "+f);
                return f;
            }
        }
        return null;
